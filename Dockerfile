...
=======
# ----------------------------------------
# Stage 1: Backend-only build
# ----------------------------------------
FROM node:18-bullseye

WORKDIR /app

# ✅ Install build dependencies for mediasoup (Debian uses apt-get!)
RUN apt-get update && \
    apt-get install -y python3 python3-pip build-essential pkg-config libssl-dev libstdc++-10-dev && \
...
# ✅ Expose backend port (Render sets $PORT dynamically)
EXPOSE 5000
